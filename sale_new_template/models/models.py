    ...
    def action_confirm(self):
        ...
        for rec in self:
            for order in rec.order_line:
                min_qty = self.env['stock.warehouse.orderpoint'].search([('product_id','=',order.product_id.id)])
                if (min_qty.qty_on_hand - order.product_uom_qty) < min_qty.product_min_qty:
                    raise ValidationError(_("You Can Not Sale This Product As A Product Quantity With Min Quantity"))
        return result
